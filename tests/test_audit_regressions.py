...
class AuditRegressionTests(unittest.TestCase):
    ...
    def test_structured_and_known_secret_values_are_redacted(self):
        ...
        with mock.patch.dict(os.environ,{"SERVICE_TOKEN":secret},clear=False):
            value=redact_data({"api_key":"raw-key","detail":f"returned {secret}","nested":{"password":"pw12345"}})
        text=json.dumps(value)
        self.assertNotIn(secret,text); self.assertNotIn("raw-key",text); self.assertNotIn("pw12345",text); self.assertIn("[REDACTED]",text)

    def test_run_request_is_redacted_before_persistence(self):
        with tempfile.TemporaryDirectory() as td:
            target=Path(td); store=ArtifactStore(target)
            with mock.patch.dict(os.environ,{"SERVICE_TOKEN":"prompt-secret-12345"},clear=False):
                run=store.create_run("deploy using token=prompt-secret-12345","lite","open","code")
            text=(run.run_dir/"REQUEST.json").read_text()
            self.assertNotIn("prompt-secret-12345",text)
    ...
    def test_run_id_rejects_path_traversal(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(ValueError): store.create_run("x","lite","open","code",run_id="../../escape")
            with self.assertRaises(ValueError): store.get_run("../escape")

    def test_model_only_override_keeps_provider_and_unavailable_provider_fails(self):
        router=AdaptiveRouter({"claude":{"available":True,"authenticated":True},"codex":{"available":False,"authenticated":False}})
        assigned=router.assign_roles(["planner"],overrides={"planner":{"model":"opus"}})
        self.assertEqual(assigned["planner"]["provider"],"claude"); self.assertEqual(assigned["planner"]["model"],"opus")
        with self.assertRaises(ValueError): router.assign_roles(["planner"],overrides={"planner":{"provider":"codex"}})
    ...
    def test_builder_cannot_overwrite_reserved_rubric(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(any(str(f.get("id","")).startswith("F-POLICY-builder") for f in runner._findings(run)))
    ...
    def test_project_adapter_detects_linked_worktree(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td)/"repo"; worktree=Path(td)/"wt"; root.mkdir()
            if subprocess.run(["git","init",str(root)],capture_output=True).returncode!=0: self.skipTest("git unavailable")
            ...
            (root/"x.txt").write_text("x"); subprocess.run(["git","-C",str(root),"add","x.txt"],check=True); subprocess.run(["git","-C",str(root),"commit","-m","init"],check=True,capture_output=True)
            ...
            self.assertTrue(info["is_repo"]); self.assertTrue(info["is_linked_worktree"])
