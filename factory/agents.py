...
BASE_RULES=["Coordinate only through declared artifacts and the orchestrator.","Never claim PASS without admissible evidence.","Do not expose secrets or copy environment values into artifacts.","Respect the existing project's instructions and structure unless the SPEC explicitly changes them."]
def A(identity,mission,capability,tools,inputs,outputs,rules=None): return {"identity":identity,"mission":mission,"capability":capability,"tools":tools,"inputs":inputs,"outputs":outputs,"rules":BASE_RULES+(rules or [])}
...
"content_evaluator":A("Independent Content Evaluator","Verify content against measurable platform, brand, factual and production criteria.","independent_review",["read","files","ffmpeg","web"],["SPEC","RUBRIC","content_artifacts"],["RUBRIC.json","FINDINGS.json","EVIDENCE"],["Must not modify the produced content during evaluation."]),
...
"fact_checker":A("Independent Fact Checker","Verify claims against sources, recency, contradictions and citation support.","independent_review",["web","browser","files"],["SPEC","RUBRIC","EVIDENCE"],["RUBRIC.json","FINDINGS.json","EVIDENCE"],["Do not rewrite the research while evaluating it."])}
class AgentRegistry:
