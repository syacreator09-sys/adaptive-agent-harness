...
def _target(args)->Path: return Path(getattr(args,"target",None) or os.getcwd()).resolve()
...
def setup(args):
    ...
    if ".aah/" not in text.splitlines(): gi.write_text(text+("\n" if text and not text.endswith("\n") else "")+".aah/\n",encoding="utf-8")
...
def doctor(args):
    ...
    payload={"ok":True,"target":str(target),"providers":providers,"tools":tools,"project":project,"billing":cfg.data["billing"]}
...
def status(args):
    ...
    state=ArtifactStore.read_json(run.run_dir,"STATE.json",{}); print(json.dumps(state,indent=2)); return 0
...
def _one_step_executor(target,profile,domain):
    cfg=AAHConfig.load(target); providers=_configured_providers(cfg,ProviderRegistry.discover()); router=AdaptiveRouter(providers); roles=_roles_for(profile,domain)
    return AgentExecutor(target,router.assign_roles(roles,cfg.data["models"]["policy"]),cfg.data["billing"]["mode"]=="subscription_only")
...
def eval_cmd(args):
    ...
    role=role_for(domain,"evaluator"); runner._ingest(run,ex.execute(role,{"mode":"evaluate_only"},ctx)); gate=runner._gate(run); report=runner._write_report(run,gate,{"manual_eval":True})
...
def fix_cmd(args):
    ...
    req=ArtifactStore.read_json(run.run_dir,"REQUEST.json",{}); domain=req.get("domain","code"); profile=req.get("profile","lite"); guardian=req.get("guardian","guarded")
    ex=_one_step_executor(target,profile,domain); runner=_runner(target,profile,ex); ctx=runner._context(run,guardian)
    canonical="builder" if profile=="lite" else "fixer"; role=role_for(domain,canonical); runner._ingest(run,ex.execute(role,{"mode":"fix_only","findings":runner._findings(run)},ctx))
    eval_role=role_for(domain,"evaluator"); runner._ingest(run,ex.execute(eval_role,{"mode":"evaluate_after_fix"},ctx)); gate=runner._gate(run); report=runner._write_report(run,gate,{"manual_fix":True})
    print(json.dumps(report,indent=2)); return 0 if gate["done"] else 2
...
def init_run(args):
    target=_target(args); providers=ProviderRegistry.discover(); route=AdaptiveRouter(providers).route(args.request); profile=args.profile if args.profile!="auto" else route["profile"]; guardian=args.guardian if args.guardian!="auto" else route["guardian"]
    run=ArtifactStore(target).create_run(args.request,profile,guardian,args.domain); payload={"run_id":run.run_id,"run_dir":str(run.run_dir),"profile":profile,"guardian":guardian,"route":route}
    print(json.dumps(payload) if args.json else json.dumps(payload,indent=2)); return 0

def gate_cmd(args):
    target=_target(args); run=_resolve_run(target,args.run_id)
    if not run: print("Run not found",file=sys.stderr); return 1
    rubric=ArtifactStore.read_json(run.run_dir,"RUBRIC.json",[]) or []; findings=ArtifactStore.read_json(run.run_dir,"FINDINGS.json",[]) or []; result=FinalGate(run.run_dir).evaluate(rubric,findings)
    print(json.dumps(result,indent=2)); return 0 if result["done"] else 2
...
def build_parser():
    ...
    rb=sub.add_parser("rollback"); rb.add_argument("run_id",nargs="?"); rb.add_argument("--target"); rb.add_argument("--apply",action="store_true"); rb.set_defaults(func=rollback)
