...
def setup(args):
    target=_target(args); target.mkdir(parents=True,exist_ok=True); aah=target/".aah"; aah.mkdir(exist_ok=True)
    manifest=ProjectAdapter(target).inspect(); providers=ProviderRegistry.discover(); tools=ToolRegistry.discover()
    ...
    codex_permissions=install_profiles(target) if providers.get("codex",{}).get("available") else {"installed":False,"reason":"codex_unavailable"}
    ...
    cfg.save(target)
    gi=target/".gitignore"; text=gi.read_text(encoding="utf-8",errors="ignore") if gi.exists() else ""
    if ".aah/" not in text.splitlines():
        gi.write_text(text+("\n" if text and not text.endswith("\n") else "")+".aah/\n",encoding="utf-8")
    print(json.dumps({"ok":True,"target":str(target),"stacks":manifest["stacks"],"providers":providers,"codex_permissions":codex_permissions,"config":str(aah/"factory.local.yaml")},indent=2))
    return 0


def doctor(args):
    target=_target(args); cfg=AAHConfig.load(target); providers=ProviderRegistry.discover(); tools=ToolRegistry.discover(); project=ProjectAdapter(target).inspect()
    ...
    if args.json: print(json.dumps(payload,indent=2))
        ...
        print("Stacks: "+(", ".join(project["stacks"]) or "unknown")); print("Billing: "+cfg.data["billing"]["mode"]); print("Ready: "+("yes" if ready else "no"))
...
def _configured_providers(cfg, discovered):
    ...
    if strategy=="claude": return {"claude":discovered.get("claude",{"available":False}),"codex":{"available":False,"reason":"disabled_by_config"}}
    if strategy=="codex": return {"claude":{"available":False,"reason":"disabled_by_config"},"codex":discovered.get("codex",{"available":False})}
    return discovered


def _require_router(target, cfg):
    providers=_configured_providers(cfg,ProviderRegistry.discover()); router=AdaptiveRouter(providers)
    if router.available(): return router
    installed=[k for k,v in providers.items() if v.get("available")]
    if installed: raise RuntimeError("Provider CLI detected but no authenticated subscription session is ready. Login to Claude Code/Codex, then run factory doctor.")
    raise RuntimeError("No provider CLI detected. Install/login to Claude Code or Codex, then run factory doctor.")


def run_cmd(args):
    target=_target(args); cfg=AAHConfig.load(target); router=_require_router(target,cfg)
    ...
    if requested_profile=="auto" and cfg.data["execution"].get("profile","auto")!="auto": requested_profile=cfg.data["execution"]["profile"]
    requested_guardian=args.guardian
    if requested_guardian=="auto" and cfg.data["guardian"].get("mode","auto")!="auto": requested_guardian=cfg.data["guardian"]["mode"]
...
def status(args):
    ...
    print(json.dumps(ArtifactStore.read_json(run.run_dir,"STATE.json",{}),indent=2)); return 0
...
def resume(args):
    ...
    if state.get("status")=="done": return report(args)
    req=ArtifactStore.read_json(run.run_dir,"REQUEST.json",{}); profile=req.get("profile","lite"); guardian=req.get("guardian","guarded")
    cfg=AAHConfig.load(target); router=_require_router(target,cfg); roles=_roles_for(profile,req.get("domain","code"))
...
def eval_cmd(args):
    target=_target(args); run=_resolve_run(target,args.run_id)
    if not run: print("Run not found",file=sys.stderr); return 1
    req=ArtifactStore.read_json(run.run_dir,"REQUEST.json",{}); domain=req.get("domain","code"); profile=req.get("profile","lite"); guardian=req.get("guardian","guarded")
    ex=_one_step_executor(target,profile,domain); runner=_runner(target,profile,ex); ctx=runner._context(run,guardian); role=role_for(domain,"evaluator")
    runner._ingest(run,ex.execute(role,{"mode":"evaluate_only"},ctx)); gate=runner._gate(run); result=runner._write_report(run,gate,{"manual_eval":True})
    print(json.dumps(result,indent=2)); return 0 if gate["done"] else 2
...
def fix_cmd(args):
    ...
    if not run: print("Run not found",file=sys.stderr); return 1
    req=ArtifactStore.read_json(run.run_dir,"REQUEST.json",{}); domain=req.get("domain","code"); profile=req.get("profile","lite"); guardian=req.get("guardian","guarded")
    ex=_one_step_executor(target,profile,domain); runner=_runner(target,profile,ex); ctx=runner._context(run,guardian); canonical="builder" if profile=="lite" else "fixer"
    runner._ingest(run,ex.execute(role_for(domain,canonical),{"mode":"fix_only","findings":runner._findings(run)},ctx)); runner._ingest(run,ex.execute(role_for(domain,"evaluator"),{"mode":"evaluate_after_fix"},ctx))
    gate=runner._gate(run); result=runner._write_report(run,gate,{"manual_fix":True}); print(json.dumps(result,indent=2)); return 0 if gate["done"] else 2
...
def rollback(args):
    ...
    if state.get("git_base_dirty") and not args.force_dirty_baseline:
        print("Rollback refused: the project already had changes at AAH start. Commit/stash them first, or use --force-dirty-baseline if you explicitly accept losing tracked baseline edits.",file=sys.stderr); return 2
...
def init_run(args):
    target=_target(args); route=AdaptiveRouter(ProviderRegistry.discover()).route(args.request); profile=args.profile if args.profile!="auto" else route["profile"]; guardian=args.guardian if args.guardian!="auto" else route["guardian"]
    store=ArtifactStore(target); run=store.create_run(args.request,profile,guardian,args.domain); snap=GitCheckpoints(target).snapshot()
    state=store.read_json(run.run_dir,"STATE.json",{}); state.update({"git_base":snap.get("head"),"git_base_dirty":snap.get("dirty",False),"git_worktree":snap.get("is_linked_worktree",False)}); store.write_json(run.run_dir,"STATE.json",state)
    req=store.read_json(run.run_dir,"REQUEST.json",{}); req["git_base"]=snap.get("head"); store.write_json(run.run_dir,"REQUEST.json",req)
    payload={"run_id":run.run_id,"run_dir":str(run.run_dir),"profile":profile,"guardian":guardian,"route":route,"git_base":snap.get("head")}; print(json.dumps(payload) if args.json else json.dumps(payload,indent=2)); return 0
...
def gate_cmd(args):
    target=_target(args); run=_resolve_run(target,args.run_id)
    if not run: print("Run not found",file=sys.stderr); return 1
    rubric=ArtifactStore.read_json(run.run_dir,"RUBRIC.json",[]); findings=ArtifactStore.read_json(run.run_dir,"FINDINGS.json",[]); result=FinalGate(run.run_dir).evaluate(rubric,findings,_native_mandatory_gates(run))
    print(json.dumps(result,indent=2)); return 0 if result["done"] else 2
...
def build_parser():
    ...
    g=sub.add_parser("gate"); g.add_argument("run_id",nargs="?"); g.add_argument("--target"); g.set_defaults(func=gate_cmd); return p
...
def main(argv=None):
    try:
        ...
        print(f"AAH error: {exc}",file=sys.stderr); return 3
