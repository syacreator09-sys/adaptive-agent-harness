...
class AutoOrchestrator:
    ...
    def __init__(self,target:Path|str,executor_factory,router:AdaptiveRouter):
        self.target=Path(target).resolve(); self.executor_factory=executor_factory; self.router=router; self.store=ArtifactStore(self.target)
    def run(self,request:str,domain="code",profile="auto",guardian="auto") -> dict[str,Any]:
        route=self.router.route(request); current=route["profile"] if profile=="auto" else profile; guard=route["guardian"] if guardian=="auto" else guardian
        chain=[]; parent=None
        while True:
            ex=self.executor_factory(current,domain)
            result=self.RUNNERS[current](self.target,ex).run(request,guardian=guard,domain=domain)
            run_id=result["run_id"]; chain.append({"profile":current,"run_id":run_id,"done":result["done"]})
            if parent:
            ...
            if result["done"] or profile!="auto":
                result["chain"]=chain; result["route"]=route; return result
