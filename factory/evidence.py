...
SECRET_VALUE_PATTERNS=[re.compile(r"(?i)(api[_-]?key|token|secret|password)\s*[:=]\s*([^\s,;]+)"),re.compile(r"\bsk-[A-Za-z0-9_-]{12,}\b")]
...
class EvidenceStore:
    ...
    def ids(self): return {str(x.get("id")) for x in self.all()}
