...
class TaskGraph:
    def __init__(self,data:dict[str,Any]): self.tasks=data.get("tasks",[]) if isinstance(data,dict) else []; self.by_id={str(t.get("id")):t for t in self.tasks}; self.validate()
    def validate(self):
