...
class ArtifactStore:
    ...
    def _next_run_id(self) -> str:
        day = datetime.now().strftime("%Y%m%d"); prefix = f"RUN-{day}-"; nums=[]
    ...
    def create_run(self, request: str, profile: str, guardian: str, domain: str, run_id: str | None = None) -> RunHandle:
        run_id=run_id or self._next_run_id(); run_dir=self.root/run_id; run_dir.mkdir(parents=True,exist_ok=False)
        for name in ["logs","screenshots","artifacts"]: (run_dir/name).mkdir()
        self.write_json(run_dir,"REQUEST.json",{"request":request,"profile":profile,"guardian":guardian,"domain":domain,"created_at":datetime.now().astimezone().isoformat()})
        self.write_json(run_dir,"STATE.json",{"run_id":run_id,"phase":"created","profile":profile,"guardian":guardian,"domain":domain,"pass":0,"status":"running","history":[]})
    ...
    def latest_run(self) -> RunHandle | None:
        runs=sorted([p for p in self.root.iterdir() if p.is_dir()]); return RunHandle(runs[-1].name,runs[-1]) if runs else None
    ...
    def write_json(self, run_dir: Path, name: str, data: Any) -> Path:
        path=self._safe_path(run_dir,name); self._atomic_write(path,json.dumps(data,indent=2,sort_keys=True)+"\n"); return path

    def write_text(self, run_dir: Path, name: str, text: str) -> Path:
        path=self._safe_path(run_dir,name); self._atomic_write(path,text if text.endswith("\n") else text+"\n"); return path

    def append_jsonl(self, run_dir: Path, name: str, data: dict[str, Any]) -> Path:
        path=self._safe_path(run_dir,name); path.parent.mkdir(parents=True,exist_ok=True)
